...
#define TABBLOCK		8			// Tab length in spaces
...
int main(void){
	...
	int nextBlock;					// The next tab block column position relative to the current character
	...
	while ((c = getchar()) != EOF){
		...
		if (c == ' '){
			if (spaces > 0){
				...
				++spaces;
				// Calculates the next tab block location, vital for outputing the correct data
				nextBlock = columnpos + (TABBLOCK - (columnpos % TABBLOCK));
			}
		}
		else{
			if (spaces > 0){
				...
				// If the spaces at least reach the next tab block
				else{
					// We're going to insert a tab to replace the spaces from the
					// 	spaceStart column to the next tab block
					...
					// Since we are starting on a tab block
					putchar('\t');
					spacesOut = (spaces % TABBLOCK);
					tabsOut = ((spaces - spacesOut) / TABBLOCK);
